    ...
    return (
        <div className={`whitespace-nowrap flex flex-col min-h-screen bg-green-medium ${expand ? "w-64" : "w-14 flex-none"}`}>
            ...
            <div className="flex flex-col h-full py-5">
                ...
                <NavLink to="IA-MARKS" className={({ isActive }) => isActive ? " bg-green-dark py-2 w-full flex flex-row" : "hover:bg-green-light py-2 w-full flex flex-row"}>
                    <p className={`font-Outfit-Medium tracking-needed text-white ${expand ? "mx-6" : "absolute left-[-999px]"}`}>IA-MARKS</p>
                    <img src={seat} alt="SA" className={`${expand ? "absolute left-[-999px]" : "h-7 w-7 ml-3 mr-4 my-1"}`} title="Seat Allocation" />
                </NavLink>
